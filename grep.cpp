...
void runsource(int pfd[])	/* run the first part of the pipeline, cmd1 */
{
    ...
    switch (pid = fork()) {
        ...
        case 0: /* child */
            dup2(pfd[1], 1);	/* this end of the pipe becomes the standard output 1 standard output*/
            //standard output redirect to pipe output
            close(pfd[0]); 		/* this process don't need the other end */
            execl("./out", NULL,NULL);	/* run the command */
...
void rundest(int pfd[])	/* run the second part of the pipeline, cmd2 */
{
    ...
    switch (pid = fork()) {
        ...
        case 0: /* child */
            dup2(pfd[0], 0);	/* this end of the pipe becomes the standard input 0 standard input */
            //standard input redirect to pipe input
            close(pfd[1]);		/* this process doesn't need the other end */
            execl("./in", NULL,NULL);	/* run the command */
